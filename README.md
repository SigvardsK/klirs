...
> Open-source AML/KYC screening portal with first-class Baltic + global coverage. Playwright-based evidence capture, tri-state detection, audit-ready PDF/DOCX export. Bring your own Supabase. AGPL-3.0.

A self-hostable web app that screens an entity (individual or company) across **8 sanctions / PEP / registry / adverse-media databases** and produces:

- A live evidence trail (timestamped screenshots + source URL per check)
- A category-weighted risk score (LOW / LOW-MEDIUM / MEDIUM / HIGH / REJECT) with a transparent breakdown
- An auditable PDF bundle suitable for compliance filing
- Pre-filled Latvian Bar Association annexes (P2 / P3.1 / P3.2) as PDF + DOCX, ready for lawyer review
...
|---|---|---|
| Firmas.lv Sankciju saraksts | Sanctions (FID consolidated — LV/EU/UN/US/UK/AU) | Substring `hitIndicator` on Latvian pagination summary |
| OFAC SDN List | US Sanctions | Regex `hitIndicatorPattern` on `Lookup Results: N Found` |
| UK Sanctions List | UK Sanctions | Substring on `records found`. Companies use `downgradeCompanyHits` flag (UK SPA tokenises multi-word queries) |
| VID PNP | Latvian PEP / tax debtors | Persons-only |
| VID VAD | Latvian officials' declarations | Persons-only |
| DuckDuckGo | Adverse media (LV/EN/ET/RU) | Persons-only; requires `playwright-extra` stealth |
| Uzņēmumu reģistrs (UR) | Company registry (LV) | 3 screenshots: Search → Detail → Persons tab (UBO at viewport 1800) |

User-supplied aliases and LV-transliteration auto-variants (`expandLvVariants` — drops/adds trailing `s`/`š`) fan out: each variant gets a full sweep across applicable sources. Persons-only sources skip the company entity. The form preview shows the exact derived list before submission; the Checks tab labels every non-original variant with an `(auto-variant)` badge so reviewers can audit fan-out.
...
Stack: **Next.js 15** (App Router) · **React 19** · **TypeScript** · **Tailwind v4** · **shadcn/ui** · **Supabase** (Postgres + Storage + OAuth) · **Playwright** (`playwright-extra` + `puppeteer-extra-plugin-stealth`) · **Railway** (Docker — Nixpacks doesn't include Chromium).
...
- **LV-jurisdiction-coupled.** The submission form's jurisdiction dropdown is currently LV / EE / LT / Other-EU only; alias auto-expansion uses LV-transliteration heuristics; the annex generator is the Latvian Bar Association schema (Instrukcija NILLTPFN-SL). Users outside LV-adjacent compliance contexts can fork or contribute a wider jurisdiction set + non-LV annex schemas.
...
A hosted (managed) version is in productization — until it ships, self-host or [open an issue](https://github.com/SigvardsK/eu-aml-screening/issues/new) to be notified. Other directions are tracked as [GitHub Issues](https://github.com/SigvardsK/eu-aml-screening/issues): jurisdiction-aware adverse-media language selection, manual-review workflow, wider jurisdiction dropdown, non-LV annex schemas. Community PRs welcome — see Contributing below.

---

## Contributing

Issues and PRs welcome. No SLA, reviewed weekly best-effort. Before opening a PR that touches `src/lib/screening-engine.ts`, `src/lib/db-configs.ts`, or `src/lib/risk-score.ts`, please run `npm run smoke:sanctioned` against your fork's deployment and include the output. The classifier contract is regression-prone in subtle ways — the load-bearing invariant is documented at the top of `screening-engine.ts`.
